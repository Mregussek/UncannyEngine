...
static glslang_target_client_version_t ParseVulkanVersion(u32 targetVulkanVersion);
...
{
	m_TargetVulkanVersion = ParseVulkanVersion(targetVulkanVersion);
...
std::vector<u32> FGLSLShaderCompiler::Compile(const char* glslSource, EShaderCompilerStage stage) const
{
	if (not m_Initialized)
	{
		AssertVkAndThrow(VK_ERROR_INITIALIZATION_FAILED, "Cannot compile shader, compiler is not initialized!");
	}
...
glslang_target_client_version_t ParseVulkanVersion(u32 targetVulkanVersion)
{
