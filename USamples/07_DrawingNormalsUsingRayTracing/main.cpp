...
private:
  ...
  void Start() {
    ...
    m_ObjectsUniformBuffer = vulkan::FBuffer(pLogicalDevice->GetHandle(), &pPhysicalDevice->GetAttributes());
    {
      ...
      m_ObjectsUniformBuffer.FillStaged(blasUniformData.data(), sizeof(blasUniformData[0]),
    ...
    m_ObjectsDescriptorSetLayout.Create(pLogicalDevice->GetHandle());

    m_ObjectsDescriptorPool.Create(pLogicalDevice->GetHandle(), &m_ObjectsDescriptorSetLayout, 1);
    ...
    vulkan::FRayTracingPipelineSpecification rayTracingPipelineSpecification{
        .rayClosestHitPath = FPath::Append(shadersSpvPath, "normals.rchit.spv"),
